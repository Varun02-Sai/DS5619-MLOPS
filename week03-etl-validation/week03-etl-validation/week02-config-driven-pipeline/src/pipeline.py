...
def load_config(path):
    ...
    for k in REQUIRED_KEYS:
        if k not in config:
            raise ValueError(f"missing variable: {k}")
...
def load_transactions(path, fmt):
    ...
    for t in trans:
        ...
        if "amount" not in t or "is_fraud" not in t:
            raise ValueError(
                'each transaction must contain "amount" and "is_fraud"'
            )
        ...
        is_fraud = t["is_fraud"]
        if not (
            isinstance(is_fraud, bool)
            or (
                isinstance(is_fraud, str)
                and is_fraud in ("True", "False")
            )
        ):
            raise ValueError(
                '"is_fraud" must be "True", "False", or a boolean'
            )
...
def run_pipeline(config):
    ...
    n_fraud = sum(
        1 for r in rows
        if str(r["is_fraud"]).lower() == "true"
    )

    n_high_value = sum(
        1
        for r in rows
        if float(r["amount"]) > config["high_value_threshold"]
    )
